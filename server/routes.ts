...
router.post('/admin/update/apply', adminAuth, (req, res) => {
  ...
  setTimeout(() => {
    ...
    const gitCmd = `${safeGit} && git fetch ${repoUrl} ${targetBranch} && git reset --hard FETCH_HEAD && npm install --include=dev && npm run build`;
    const tarballCmd = `(curl -sL https://codeload.github.com/Lolergags/ChuniTrrracker/tar.gz/refs/heads/${targetBranch} 2>/dev/null || wget -qO- https://codeload.github.com/Lolergags/ChuniTrrracker/tar.gz/refs/heads/${targetBranch}) | tar -xz --strip-components=1 -C ${process.cwd()} && npm install --include=dev && npm run build`;
