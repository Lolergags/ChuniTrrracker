...
router.get('/leaderboard', (req, res) => {
  ...
  const totalActiveSongs = (db.prepare(`
    ...
    WHERE c.difficulty != 'WE' AND c.id NOT IN (95, 201, 239116) ${serverCondition}
    ${versionFilter}
  ...
  const playersQuery = `
    ...
    LEFT JOIN (
      ...
      JOIN songs on c.song_id = songs.id
      WHERE c.difficulty != 'WE' AND c.id NOT IN (95, 201, 239116) ${serverCondition} ${versionFilter}
    ...
    LEFT JOIN (
      ...
      FROM charts c
      JOIN charts c2 ON c.song_id = c2.song_id AND c2.difficulty != 'WE' AND c2.id NOT IN (95, 201, 239116)
      JOIN songs on c.song_id = songs.id
      WHERE c.difficulty != 'WE' AND c.id NOT IN (95, 201, 239116) ${serverCondition} ${versionFilter}
      GROUP BY c.song_id
  ...
  for (let i = 0; i <= selectedVersionIdx; i++) {
    ...
    const masUlt = (db.prepare(`SELECT COUNT(*) as count FROM charts c JOIN songs ON c.song_id = songs.id WHERE c.difficulty IN ('MAS', 'ULT') ${serverCondition} AND songs.version IN (${p}) AND c.id NOT IN (95, 201, 239116)`).get(...allowed) as any).count;
    ...
    const maxOpQuery = db.prepare(`
      SELECT IFNULL(SUM(((max_const * 5000 + 15000) / 5) * 5), 0) as total_op FROM (
        ...
        FROM charts c
        JOIN songs ON c.song_id = songs.id
        WHERE c.difficulty != 'WE' ${serverCondition} AND songs.version IN (${p}) AND c.id NOT IN (95, 201, 239116)
        GROUP BY c.song_id
      )
    `).get(...allowed) as any;
    
    const activeSongs = (db.prepare(`SELECT COUNT(DISTINCT song_id) as count FROM charts c JOIN songs ON c.song_id = songs.id WHERE c.difficulty != 'WE' ${serverCondition} AND songs.version IN (${p}) AND c.id NOT IN (95, 201, 239116)`).get(...allowed) as any).count;
