...
*   **Pull Request Merges:** NEVER perform direct local `git merge` into `main`. Feature branch merges into `main` MUST ONLY be performed via a GitHub Pull Request using GitHub CLI (`gh pr create` and `gh pr merge`) when the user EXPLICITLY requests or approves merging into `main`. Do NOT automatically merge branches or PRs into `main` without explicit user instruction.
