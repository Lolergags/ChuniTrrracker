...
*   **Scatter Plot Zooming, Axis Adjustments & Ticks:** 
    *   **Synchronized Aspect Ratio Reset:** When plotting scatter data where the X-axis domain span (level constants ~3.0 wide) is significantly smaller than the Y-axis domain span (scores ~35,000 wide), unconstrained wheel zoom-out causes $X$ to hit its default domain limit early, freezing horizontal scaling while $Y$ continues zooming vertically. Always lock wheel zoom-out completion ratio using `shouldResetZoomOut(curSpanX, curSpanY, defSpanX, defSpanY, zoomFactor)` so both $X$ and $Y$ reset to unzoomed domains simultaneously when reaching 95% threshold.
