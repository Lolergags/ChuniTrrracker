...
*   **Scatter Plot Zooming, Axis Adjustments & Ticks:** 
...
*   **Mobile Touch Panning & Drag Locks:** Mobile touch devices hijack single-finger touch gestures for native page scrolling. To ensure touch drag panning works smoothly on mobile: (1) apply `touchAction: 'none'` to the scatter chart container element, (2) call `e.preventDefault()` inside `handleTouchStart` for single-finger touches (`e.touches.length === 1`), and (3) attach `touchmove`, `touchend`, and `touchcancel` listeners to `window` with `{ passive: false }` so dragging past chart bounds continues tracking accurately.
...
*   **Pull Request Merges & Branch Isolation:** NEVER perform direct local `git merge` into `main` or automatically back-merge/push feature branch commits into `main` after working on a feature branch (e.g. `graph-adjustments`, `chunithm-style`), even if `main` was previously merged into the feature branch. Feature branch changes MUST remain strictly isolated on their feature branch until the user EXPLICITLY requests to open a Pull Request and merge into `main`.
