...
import type { ApiProcessedScore, ApiHeatmapData, ApiChartMeta } from '../lib/types/index.js';
...
const PerformanceAnalysis: React.FC = () => {
  ...
  // Transform data for personal scatter plot: x = Score, y = Constant, z = OP (size)
  ...
  return (
    <div className="container animate-fade-in" style={{ padding: '2rem 0' }}>
      ...
      {/* Personal Meta Section */}
      <div style={{ marginTop: '4rem' }}>
        <h2 className="text-gradient" style={{ marginBottom: '0.5rem', fontSize: '1.8rem' }}>Personal Performance</h2>
        <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>
          Visualize your top 500 plays. Correlation between Score and Chart Constant.
        </p>

        {!activePlayer && <p style={{ color: 'var(--text-secondary)' }}>Select a player to view personal performance analysis.</p>}
        {isLoadingPlayer && <p style={{ color: 'var(--text-secondary)' }}>Loading personal visualization...</p>}

        {!isLoadingPlayer && activePlayer && scores.length > 0 && (
          <div className="glass-panel" style={{ height: '500px', width: '100%', minWidth: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                <XAxis 
                ...
                />
                <Scatter name="Scores" data={personalChartData} fill="var(--accent-primary)" fillOpacity={0.6} />
              </ScatterChart>
            </ResponsiveContainer>
          </div>
        )}
