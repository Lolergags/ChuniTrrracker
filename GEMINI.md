...
*   **Pull Request Merges & Branch Isolation:** When completing commits on a feature branch, ALWAYS push the commits to the corresponding feature branch on GitHub (`git push origin <branch-name>`) so remote tracking and deployment managers reflect the update. However, NEVER perform direct local `git merge` into `main` or automatically back-merge/push feature branch commits into `main` (e.g. `graph-adjustments`, `chunithm-style`), even if `main` was previously merged into the feature branch. Feature branch changes MUST remain strictly isolated on their feature branch until the user EXPLICITLY requests to open a Pull Request and merge into `main`.
*   **GitHub Ruleset & Branch Protection (`Protect Main Branch`):** The repository enforces active GitHub Ruleset `Protect Main Branch` (ID: 20353275) on `refs/heads/main`. Direct branch deletion and non-fast-forward force-pushes are blocked. All updates targeting `main` MUST proceed through a Pull Request (`gh pr create`) and merge request (`gh pr merge`).
